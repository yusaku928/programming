...
Pythonには最初からさまざまなコレクションが用意されている．だから，コレクションを作り方ではなく，使い方を学ぶのが大事
...
学生得点 = {'A君':90, 'B君':80, 'C君':100}
```


目標：次の四つを区別し，それぞれについて，(1) create，(2) read，(3) update，(4) delete，つまりCRUDの方法がわかればよい。（追加ができない場合もある。）

```python
a = [2, 3, 3, 2, 2, 1]                                        #リスト
b = (2, 3, 3, 2, 2, 1)                                        #タプル
c = {2, 3, 3, 2, 2, 1}                                        #セット
d = {'Alice':2, 'Bob':3, 'Taro':3, 'ABC':2, 'PQR':2, 'XYZ':1} #辞書
```
...
**問題** `myList = ['AAA', 'BBB', 10, 20]`について，次の操作を実行せよ．
...
**問題** `list_test`を大きい順に並び替えよ。（これまでに学んだことの組み合わせでできる。元のリストを保存する場合と保存しない場合を試せ．）

**問題（オプショナル）** `list_os = ['windows','mac','linux','BeOS']`でメソッド`sort`を使った結果を調べよ。

`list_os = ['Windows','Mac','linux','BeOS']`の場合はどうか。

**問題（オプショナル）** `list_os = ['Windows','Mac','linux','BeOS']`をアルファベット順に並び替えよ。

**問題（オプショナル）** リストのメソッド`append`の機能がわかる例を作れ。（空のリスト`[]`から始めるとよい。）

**問題（オプショナル）** `[9, 2, 5]`というリストをもとに`[9, 2, 5, 4, 1]`というリストを作る方法を三つ考えよ。
...
* セット: `c = {2, 3, 3, 2, 2, 1}`
  ...
  * delete:
* 辞書
  * create: `d = {'Alice':2, 'Bob':3, 'Taro':3, 'ABC':2, 'PQR':2, 'XYZ':1}`
  * read:
  * update:
  * delete:
